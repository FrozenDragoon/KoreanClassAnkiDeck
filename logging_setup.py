...
def logging_setup(
    ...
    if logToFile:
        file_handler = logging.FileHandler(
            os.path.join(logPath, f'{datetime.datetime.now().strftime("%Y%m%d")}.log')
        )

    stream_handler = logging.StreamHandler()

    formatter = logging.Formatter(
        "%(asctime)s :: %(levelname)-8s :: %(funcName)-23s - %(message)s"
    )
    if logToFile:
        # print("Setting Formatter")
    ...
    coloredlogs.install(
        level=logger.level,
        logger=logger,
        fmt="%(asctime)s :: %(levelname)-8s :: %(funcName)-23s - %(message)s",
        field_styles={
            "asctime": {"color": "green"},
...
def log_level_type(log_level: int) -> int:
    return log_level


class LoggingArgs(Tap):
    log_level: int | str = logging.INFO  # 'info', 'warning', 'debug', etc
    logToFile: bool = False

    def configure(self) -> None:  # noqa: ANN101
        self.add_argument("--log-level", type=log_level_type)

    def process_args(self) -> None:  # noqa: ANN101
        # Validate arguments
...
if __name__ == "__main__":
    ...
    logging_setup(logLevel=logArgs.log_level, logToFile=logArgs.logToFile)  # type: ignore [arg-type]
